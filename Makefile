...
build-win:    ## Build the Windows executable.
	...
	uv run pyinstaller --onefile src/main.py --name accel --add-data "config.yaml;." --hidden-import accel.cli.cli --hidden-import accel.gui.gui
	rm -rf release
	mkdir release
	cp dist/accel.exe release
...
build-macos:    ## Build the macOS executable.
	...
	uv run pyinstaller --windowed --name "TemplateApp" src/main.py --add-data "config.yaml:." --hidden-import accel.cli.cli --hidden-import accel.gui.gui
